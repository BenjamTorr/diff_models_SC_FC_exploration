...
class MyUNet(nn.Module):
    def __init__(self, n_steps=1000, time_emb_dim=100, feat_dim = 1):
        ...
        self.feat1 = nn.Sequential(
            MyBlock((1, 68, 68), 1, 10),
            MyBlock((10, 68, 68), 10, 10),
            MyBlock((10, 68, 68), 10, 10)
        )
        ...
        self.downcond1 = nn.Conv2d(10, 10, 4, 2, 1)

        self.te2 = self._make_te(time_emb_dim, 10)
        self.feat2 = nn.Sequential(
            MyBlock((10, 34, 34), 10, 20),
            MyBlock((20, 34, 34), 20, 20),
            MyBlock((20, 34, 34), 20, 20)
        )
        ...
        self.downcond2 = nn.Conv2d(20, 20, 4, 2, 1)

        self.te3 = self._make_te(time_emb_dim, 20)
        self.feat3 = nn.Sequential(
            MyBlock((20, 17, 17), 20, 40),
            MyBlock((40, 17, 17), 40, 40),
            MyBlock((40, 17, 17), 40, 40)
        )
        ...
        self.feat4 = nn.Sequential(
            MyBlock((80, 17, 17), 80, 40),
            MyBlock((40, 17, 17), 40, 20),
            MyBlock((20, 17, 17), 20, 20)
        )
        ...
        self.upcond2 = nn.ConvTranspose2d(20, 20, 4, 2, 1)

        self.te5 = self._make_te(time_emb_dim, 40)
        self.feat5 = nn.Sequential(
            MyBlock((40, 34, 34), 40, 20),
            MyBlock((20, 34, 34), 20, 10),
            MyBlock((10, 34, 34), 10, 10)
        )
        ...
        self.upcond3 = nn.ConvTranspose2d(10, 10, 4, 2, 1)
        
        self.te_out = self._make_te(time_emb_dim, 20)
        self.feat_out = nn.Sequential(
            MyBlock((20, 68, 68), 20, 10),
            MyBlock((10, 68, 68), 10, 10),
            MyBlock((10, 68, 68), 10, 10, normalize=False)
        )
    ...
    def forward(self, x, t, cond_info):
        ...
        out2 = self.b2(self.down1(out1) + self.te2(t).reshape(n, -1, 1, 1) + down_cond1)  # (N, 20, 14, 14)
        out_cond2 = self.feat2(down_cond1)
        down_cond2 = self.downcond2(out_cond2)

        out3 = self.b3(self.down2(out2) + self.te3(t).reshape(n, -1, 1, 1) + down_cond2)  # (N, 40, 7, 7)
        out_cond3 = self.feat3(down_cond2)
        down_cond3 = self.downcond3(out_cond3)

        out_mid = self.b_mid(self.down3(out3) + self.te_mid(t).reshape(n, -1, 1, 1) + down_cond3)  # (N, 40, 3, 3)
        ...
        out4 = self.b4(out4 + self.te4(t).reshape(n, -1, 1, 1) + out_cond4)  # (N, 20, 7, 7)
        out_cond4 = self.feat4(out_cond4)
        up_cond2 = self.upcond2(out_cond4)

        out_cond5 = torch.cat((out_cond2, up_cond2), dim=1)
        out5 = torch.cat((out2, self.up2(out4)), dim=1)  # (N, 40, 14, 14)
        out5 = self.b5(out5 + self.te5(t).reshape(n, -1, 1, 1) + out_cond5)  # (N, 10, 14, 14)
        ...
        out = self.b_out(out + self.te_out(t).reshape(n, -1, 1, 1) + out_cond)  # (N, 1, 28, 28)
