    ...
    for i in range(batch):
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                colorscale="Viridis",
                zmin=-1, zmax=1,
                colorbar=dict(title="Real" if i == 0 else "", len=0.33)
            ),
            row=i + 1, col=1
        )
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                y=labels,
                colorscale="Viridis",
                zmin=predicted_matrices[i].min(), zmax=predicted_matrices[i].max(),
                colorbar=dict(title="Predicted" if i == 0 else "", len=0.33)
            ),
            row=i + 1, col=2
        )
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                y=labels,
                colorscale="Viridis",
                zmin=condition_matrices[i].min(), zmax=condition_matrices[i].max(),  # Shared scale for condition matrices
                colorbar=dict(title="Condition" if i == 0 else "", len=0.33)
            ),
            row=i + 1, col=3
        )
